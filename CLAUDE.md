...
- `src/components/site-chrome.tsx` — nav (sticky, with a hamburger sheet under `md`), footer, page primitives (`PageIntro`, `LinkCard`, `BTN_PRIMARY`/`BTN_GHOST`), and the deployment map for sibling apps: `DEMO_URL` (quoting.transpiralabs.com), `PLATFORM_URL` (platform.transpiralabs.com), `BUILD_URL` (build.transpiralabs.com), `CONTACT_EMAIL`, `CAL_URL`.
